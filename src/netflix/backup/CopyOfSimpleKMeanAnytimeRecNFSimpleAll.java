...
/************************************************************************************************/
public class CopyOfSimpleKMeanAnytimeRecNFSimpleAll
{
	...
	private int								myTotalFoldsForGSU;	 //if no gsu is found for a fold, then do not include that fold in the prediction MAE etc of gsu
...
/************************************************************************************************/
    ...
    public CopyOfSimpleKMeanAnytimeRecNFSimpleAll()    
    {
 ...
 */
    ...
    public void callKTree(int callNo )     
    {
    	...
    	{
	    	timer.start();	              
	        nSpheres= simpleKTree.cluster(callNo, simVersion);       
	        timer.stop();
...
/************************************************************************************************/
    ...
    public double recommendSphere(int activeUser, int targetMovie, int neighbours)    
    {
     ...
     // variable for priors, and sim * priors
       ...
       //  if (KMeansOrKMeansPlus == 1)
         {
       		simpleKUsers = simpleKTree.getSphereByUID(activeUser);                  //simpleK tree users
       		...
       		double simWithMainCluster = simpleKTree.findSimWithOtherSphere(activeUser, activeClusterID );
       		...
       		for(int i=0;i<nSpheres; i++)
       		{
       			...
       			{
       				double activeUserSim  = simpleKTree.findSimWithOtherSphere(activeUser, i );
       		...
       		//	System.out.println(" weight ("+ (clusterId)+") with i= " + i + ",-->"+ clusterWeight);
					...
					double clusterRating = simpleKTree.getRatingForAMovieInASphere(clusterId, targetMovie);
					double clusterAverage = simpleKTree.getAverageForAMovieInASphere(clusterId, targetMovie);
	...
	*/	  
	...
	//	  String base = "C:/Users/Musi/workspace/MusiRecommender/DataSets/SML_ML/SVD/sml_TrainSetStored.dat";
		  ...
		  //  path= "/home/mag5v07/workspace/MusiRecommender/DataSets/NF/";
		    ...
		    //create class object
		    CopyOfSimpleKMeanAnytimeRecNFSimpleAll rec = new CopyOfSimpleKMeanAnytimeRecNFSimpleAll();
...
//      System.out.println(" List of user in RSME ......"+users);
      ...
      for (int i = 0; i < totalUsers; i++)        
      {
		...
		int activeClusterID = simpleKTree.getSphereIDByUID(uid);
...
//		if(go==true)
       	{   			
				//increase the gray sheep samples, we make prediction for
				 int sizeOfCluster = simpleKTree.getSphereSizeByID(activeClusterID);
